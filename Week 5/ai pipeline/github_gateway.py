...
Write a concise, helpful technical comment or acknowledgement."""
    ...
    elif getattr(config, "GITHUB_AUTO_REPLY", False) and notif.get("comments_url"):
...
_cron_jobs: dict[str, dict] = {
    "poll_notifications": {
        ...
        "schedule": f"every {getattr(config, 'GITHUB_POLL_INTERVAL', 60)}s",
        "enabled":  True,
        "last_run": None,
        "next_run": None,
    },
...
def run_daily_summary() -> None:
    stats = mem.get_stats()
    prompt = f"""Generate a brief daily GitHub activity summary for {getattr(config, 'OWNER_NAME', 'User')}:
...
def api_status():
    return jsonify({
        ...
        "auto_reply":     getattr(config, "GITHUB_AUTO_REPLY", False),
        "model":          getattr(config, "OLLAMA_MODEL", "llama3.2:3b"),
        "poll_interval":  getattr(config, "GITHUB_POLL_INTERVAL", 60),
...
def api_config():
    return jsonify({
        "owner":          getattr(config, "OWNER_NAME", "User"),
        "username":       getattr(config, "GITHUB_USERNAME", "unknown"),
        "model":          getattr(config, "OLLAMA_MODEL", "llama3.2:3b"),
        "poll_interval":  getattr(config, "GITHUB_POLL_INTERVAL", 60),
        "dry_run":        getattr(config, "DRY_RUN", False),
        "auto_reply":     getattr(config, "GITHUB_AUTO_REPLY", False),
        "gateway_port":   getattr(config, "GITHUB_GATEWAY_PORT", 5001),
    })
...
def run():
    ...
    log.info("Owner     : %s", getattr(config, "OWNER_NAME", "User"))
    log.info("Username  : %s", getattr(config, "GITHUB_USERNAME", "unknown"))
    log.info("Model     : %s", getattr(config, "OLLAMA_MODEL", "llama3.2:3b"))
    log.info("Poll      : every %ds", getattr(config, "GITHUB_POLL_INTERVAL", 60))
    log.info("DryRun    : %s", getattr(config, "DRY_RUN", False))
    log.info("AutoReply : %s", getattr(config, "GITHUB_AUTO_REPLY", False))
    log.info("=" * 60)

    _backend = GitHubBackend(username=getattr(config, "GITHUB_USERNAME", "unknown"))

    print(f"\n  ✅  LocalClaw GitHub Gateway is online")
    print(f"     Username  : {getattr(config, 'GITHUB_USERNAME', 'unknown')}")
    print(f"     Model     : {getattr(config, 'OLLAMA_MODEL', 'llama3.2:3b')}")
    print(f"     Dashboard : http://{getattr(config, 'GITHUB_GATEWAY_HOST', '127.0.0.1')}:{getattr(config, 'GITHUB_GATEWAY_PORT', 5001)}")
    print()
    ...
    app.run(
        host=getattr(config, "GITHUB_GATEWAY_HOST", "127.0.0.1"),
        port=getattr(config, "GITHUB_GATEWAY_PORT", 5001),
        debug=getattr(config, "GITHUB_GATEWAY_DEBUG", False),
        use_reloader=False,
    )
